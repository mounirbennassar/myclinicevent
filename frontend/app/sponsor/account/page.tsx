...
export default function SponsorAccountPage({ searchParams }: { searchParams: Promise<{ required?: string }> }) {
  const { required } = use(searchParams);
  const { user } = useSponsor();
  const { t } = useI18n();
      ...
      <Card className="flex items-center gap-4 p-5">
        ...
        <div>
          ...
          <div className="text-[13px] text-ink-500">
            {user.email} · {t.roles.sponsor}
          </div>
        </div>
      </Card>
      <div className="mt-5">
        <ChangePasswordForm onChanged={() => mustChange && router.replace("/sponsor")} />
      </div>
    </div>
  );
}
